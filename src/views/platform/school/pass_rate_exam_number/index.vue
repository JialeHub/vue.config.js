<template>
  <div id="pass_rate_exam_number_school">
    <v-sheet elevation="0" color="rgba(255,255,255,0)">
      <!--筛选栏-->
      <div class="filterToolbar" v-show="filterToolbar">
        <v-text-field label="教练名称" hint="教练名称" style="flex: 0 1 200px" dense
        ...
        <v-menu v-model="FilterMenuRegisterDate" :close-on-content-click="false" transition="scale-transition" offset-y
          ...
          <template v-slot:activator="{ on, attrs }">
            <v-text-field label="考试日期范围" style="flex: 0 1 290px" outlined dense readonly hide-details
                          v-model="searchOption['subject23_exam.date']" v-bind="attrs" v-on="on" clearable
            />
          </template>
          <v-date-picker first-day-of-week="1" range v-model="searchOption['subject23_exam.date']" clearable></v-date-picker>
        </v-menu>

        <autocomplete-area v-model="searchOption['school.area']" outlined label/>

        <autocomplete-school v-model="searchOption.schoolId" :area="searchOption['school.area']" outlined dense @change="search()"/>
      ...
      <v-row>
        <v-col cols="auto">
          <div class="batchOperationBtn">
            ...
            <!--              新增-->
            <!--            </v-btn>-->
            <!--            <v-btn depressed color="warning" small :href="$addBaseURL(`/coach/export`)" target="_blank">-->
      ...
      <!--   数据列表   -->
      <v-data-table elevation="1" class="coachTable dataTable"  fixed-header multi-sort show-select
                ...
                <v-container>
                  <v-form :disabled="loadingSubmit" v-model="formRules.formValid" :lazy-validation="true" ref="form">
                    <v-row>
                      ...
                      <v-col cols="6">
                        <autocomplete-school v-model="form.schoolId" :area="form['school.area']" outlined dense :rules="formRules.schoolIdRules"/>
                      </v-col>

                      <!--教练名称-->
                      <v-col cols="12">
                        <v-text-field v-model="form.name" label="教练名称"  :rules="formRules.nameRules" dense></v-text-field>
                      </v-col>
                      <!--序号-->
<!--                      <v-col cols="12">-->
...
<!--                      </v-col>-->
                      ...
                      <!--手机号-->
                      <v-col cols="12">
                        <v-text-field v-model="form.phone" label="手机号" :rules="formRules.phoneRules" dense></v-text-field>
                      </v-col>
                      <!--状态-->
                      <!--                      <v-col cols="12">-->
        ...
        <template v-slot:[`item.subject2ExamPassRate`]="{ item }">
          <span v-if="item.subject2ExamPassRate">{{item.subject2ExamPassRate}}%</span>
          <span v-else>无</span>
        </template>
        <template v-slot:[`item.subject3ExamPassRate`]="{ item }">
          <span v-if="item.subject3ExamPassRate">{{item.subject3ExamPassRate}}%</span>
          <span v-else>无</span>
        </template>
        <template v-slot:[`item.subject23ExamPassRate`]="{ item }">
          <span v-if="item.subject23ExamPassRate">{{item.subject23ExamPassRate}}%</span>
          <span v-else>无</span>
        </template>
...
<script>
import qs from "qs";
import {addCoachApi, delCoachApi, editCoachApi, readCoachApi, coachListApi} from "@/api/modules";
...
export default {
  name: 'pass_rate_exam_number_school',
  components: {AutocompleteSchool, AutocompleteArea},
  data() {
    return {
      type: 0,
      test: '',
      ...
      headers: [
        ...
        {text: '序号', value: 'subject23ExamPassRateRank', initHide: true,sortable: false,},//initHide
        // {text: '序号', value: 'sort', initHide: false},
        {text: '编号', value: 'code', initHide: false},
        {text: '教练名称', value: 'name', initHide: false},
        {text: '手机号', value: 'phone', initHide: true},//initHide
        {text: '所属驾校', value: 'school_name', initHide: true},//initHide
        {text: '驾校ID', value: 'school_id', initHide: true},
        {text: '驾校地区', value: 'school_area', initHide: true},//initHide
        {text: '驾校序号', value: 'school_sort', initHide: true},
        {text: '驾校编号', value: 'school_code', initHide: true},
        {text: '状态', value: 'status', initHide: true},

        {text: '科二考试人/次数', value: 'subject2ExamCount', initHide: false},
        {text: '科二考试及格率', value: 'subject2ExamPassRate', initHide: false},
        {text: '科三考试人/次数', value: 'subject3ExamCount', initHide: false},
        {text: '科三考试及格率', value: 'subject3ExamPassRate', initHide: false},
        {text: '考试总人/次数', value: 'subject23ExamCount', initHide: true},//initHide
        {text: '考试总及格率', value: 'subject23ExamPassRate', initHide: true},//initHide
      ...
      searchSqlType: {
        ...
        strLike: ['name','phone', 'code', 'school.name',  'createUser.username'],/* 模糊查询 */
        dateBetween: ['createTime', 'updateTime','subject23_exam.date'],/* 日期范围查询 */
      },

      formRules: {
        autoValid: true,
        formValid: true,
        schoolIdRules: [v => this.formRules.autoValid || !!v || '请选择驾校'],
        ...
        nameRules: [v => this.formRules.autoValid || !!v || '请输入教练名'],
      },



      infoSchool:{},
      readLoadingSchool:false,
    }
  },
  ...
  methods: {
    ...
    async fetchData() {
      ...
      data.searchList && (data.searchList = data.searchList.filter(item=>!(item[0] === 'school.area' && item[2] === '全国' )))
    ...
    async readDataSchool(id) {
      if (this.readLoadingSchool) return;
      let data = {id}
      this.readLoadingSchool = true
      let res = await readCoachApi(data).catch(err => err)
      this.readLoadingSchool = false
      this.infoSchool = res.info
      return res;
    },
    /* ******************删除数据****************** */
    ...
    async deleteItemConfirm() {
      ...
      let res = await delCoachApi(data).then(res => res).catch(err => err).finally(() => {
        this.fetchData();
      })
    ...
    async openFormDialog(item) {
      ...
      if (this.$notEmpty(item) && this.$notEmpty(item['id'])) {
        ...
        res.info.school && (this.form['school.area']=res.info.school.area)
      }else{
        //新增
        this.form.schoolId=this.searchOption.schoolId
    ...
    async submitForm() {
      ...
      let submitFun = this.$notEmpty(data.id) ? editCoachApi : addCoachApi;
