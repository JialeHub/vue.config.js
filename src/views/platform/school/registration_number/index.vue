<template>
  <div id="registration_number_school">
    <v-sheet elevation="0" color="rgba(255,255,255,0)">
      ...
      <div class="filterToolbar" v-show="filterToolbar">
        ...
        <v-menu v-model="FilterMenuRegisterDate" :close-on-content-click="false" transition="scale-transition" offset-y
          ...
          <template v-slot:activator="{ on, attrs }">
            <v-text-field label="报名日期范围" style="flex: 0 1 290px" outlined dense readonly hide-details
                          v-model="searchOption['user.register_date']" v-bind="attrs" v-on="on" clearable
            />
          </template>
          <v-date-picker first-day-of-week="1" range v-model="searchOption['user.register_date']" clearable></v-date-picker>
        </v-menu>
        ...
        <v-btn depressed color="warning" @click="commitOption({sortBy:['registerCount'],sortDesc:['true']})">
...
export default {
  name: 'registration_number_school',
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: '手机号', value: 'phone', initHide: false},
        {text: '所属驾校', value: 'school_name', initHide: false},
        {text: '驾校ID', value: 'school_id', initHide: true},
        {text: '驾校地区', value: 'school_area', initHide: false},
        {text: '驾校序号', value: 'school_sort', initHide: true},
        {text: '驾校编号', value: 'school_code', initHide: true},
        {text: '状态', value: 'status', initHide: true},
        {text: '报名人数', value: 'registerCount', initHide: false},
      ...
      searchSqlType: {
        ...
        dateBetween: ['createTime', 'updateTime','user.register_date'],/* 日期范围查询 */
      },
  ...
  beforeRouteEnter(to,from,next){
    if(!to.query.option){next({...to,query: {option: {sortBy:['registerCount'],sortDesc:['true']}}})
    }else{
      next()
    }
  },
