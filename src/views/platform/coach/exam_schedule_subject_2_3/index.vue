<template>
  <div id="exam_schedule_subject_2_3_coach">
    <v-sheet elevation="0" color="rgba(255,255,255,0)">
      ...
      <div class="filterToolbar" v-show="filterToolbar">
        ...
        <v-select style="flex: 0 1 200px" outlined dense :menu-props="{ offsetY: true }"
                  :items="[{text:'待处理',value:'0'},{text:'已通过',value:'1'},{text:'未通过',value:'2'}]" v-model="searchOption.passStatus" clearable
                  label="通过状态" hint="通过状态" hide-details
        ></v-select>
      ...
      <v-row>
        <v-col cols="auto">
          <div class="batchOperationBtn">
            ...
            <!--            <v-btn depressed color="warning" small :href="$addBaseURL(`/subject23Exam/export`)" target="_blank">-->
      ...
      <!--   数据列表   -->
      <v-data-table elevation="1" class="subject23ExamTable dataTable" fixed-header multi-sort show-select
        ...
        <template v-slot:top>
          ...
          <v-dialog persistent v-model="formDialog" max-width="600px">
            <v-card>
              <v-card-title>
                <span class="headline">{{ $notEmpty(form.id) ? '编辑科二科三考试安排' : '新增科二科三考试安排' }}</span>
              </v-card-title>
              ...
              <v-card-text>
                <v-container>
                  <v-form :disabled="loadingSubmit"  v-model="formRules.formValid" :lazy-validation="true" ref="form">
                    ...
                    <v-row>
                      <v-col cols="6">
                        <autocomplete-area v-model="form['school.area']" outlined label :disabled="!!form.id"/>
                      </v-col>
                      <v-spacer/>
                      <v-col cols="6">
                        <autocomplete-school v-model="form['school.id']" :area="form['school.area']" outlined dense :disabled="!!form.id"/>
                      </v-col>
                    </v-row>
                    <v-row >
                      <v-col cols="6">
                        <autocomplete-coach v-model="form['coach.id']" :schoolArea="form['school.area']" :schoolId="form['school.id']" outlined dense :disabled="!!form.id"/>
                      </v-col><v-spacer/>
                      <v-col cols="6">
                        <autocomplete-student v-model="form.userId" :schoolArea="form['school.area']" :schoolId="form['school.id']" :coachId="form['coach.id']" outlined dense :rules="formRules.userIdRules" :disabled="!!form.id"
                                              :noExam="0" :statusSubject="['<','4']" @getItem="userSelectFormChange"/>
                      </v-col>
                    </v-row>
                    <!--考试科目-->
                    <v-row >
                      <v-col cols="12">
                        <v-select outlined dense :menu-props="{ offsetY: true }"
        ...
        <template v-slot:[`item.subject_type`]="{ item }">
          <v-chip color="primary" v-if="item.subject_type<3" small label>科目二</v-chip>
          <v-chip color="success" v-else-if="item.subject_type>2" small label>科目三</v-chip>
        </template>
        <template v-slot:[`item.pass_status`]="{ item }">
          <v-btn color="success" v-if="item.pass_status===0" small class="mr-1" :loading="(readLoading && info.id===item.id)" @click="changePassStatus(1,item)">通过</v-btn>
          <v-btn color="error" v-if="item.pass_status===0" small :loading="(readLoading && info.id===item.id)" @click="changePassStatus(2,item)">不通过</v-btn>

          <v-chip color="success" v-if="item.pass_status===1" small label>已通过</v-chip>
          <v-chip color="error" v-if="item.pass_status===2" small label>未通过</v-chip>
        </template>
...
import {
  addSubject23ExamApi,
  delSubject23ExamApi,
  editSubject23ExamApi,
  readSubject23ExamApi,
  subject23ExamListApi
} from "@/api/modules";
import qs from "qs";
...
export default {
  name: 'exam_schedule_subject_2_3_coach',
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: '考试科目', value: 'subject_type',initHide:false,groupable: false},
        ...
        {text: '当前通过科目', value: 'user_statusSubject',initHide:true,groupable: false},
        {text: '是否通过', value: 'pass_status',width:'180px', align: 'center', initHide: false,groupable: false},
        {text: '操作', value: 'allowAction',width:'110px', sortable: false, align: 'center',groupable: false},
      ],
      ...
      searchSqlType: {
        equal: ['status', 'userId', 'coach.id', 'school.area', 'school.id','passStatus','subjectType'],/* 精确查询 */
      ...
      formRules: {
        ...
        subjectTypeRules: [v => this.formRules.autoValid || !!v || '请选择能参加考试的学员'],
      },
    }
  },
  ...
  methods: {
    async changePassStatus(passStatus,item){
      this.readLoading=true
      this.info=item
      let data = {passStatus,id:item.id}
      let res = await editSubject23ExamApi(data).then(res => res).catch(err => err)
      this.info={}
      this.readLoading=false
      if(res['status'] === 200) item.pass_status=passStatus
    },
    ...
    async fetchData() {
      ...
      this.loading = true
      let res = await subject23ExamListApi(data).catch(err => err)
      this.loading = false
      if (res.status === 200) {
        let list = [...res.list]
    ...
    async readData(item) {
      ...
      this.info = item
      this.readLoading = true
      let res = await readSubject23ExamApi(data).catch(err => err)
      this.readLoading = false
      this.info = res.info
      return res;
    },
    ...
    async deleteItemConfirm() {
      ...
      let res = await delSubject23ExamApi(data).then(res => res).catch(err => err).finally(() => {
        this.fetchData();
      })
    ...
    async submitForm() {
      ...
      let submitFun = this.$notEmpty(data.id) ? editSubject23ExamApi : addSubject23ExamApi;
...
<style lang="scss">
#exam_schedule_subject_2_3_coach {
  height: 100%;
