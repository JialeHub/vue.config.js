<template>
  <v-container fluid class="loginContainer">
    ...
    <v-row no-gutters class="loginRow" justify="center">
      <h1>
<!--        <router-link to="/login" class="logoLink">{{title}}</router-link>-->
        <router-link to="/login">{{title}}<br> 后台管理系统 <br></router-link>
      </h1>
    </v-row>
    <!--提示框-->
...
<!--            </v-text-field>-->
          ...
          <div style="width: 100%;" v-if="model===1" key="forgetForm">
            ...
            <v-row no-gutters align="center" style="margin-top: 10px;">
              <v-btn depressed text color="primary" @click="reSendForgetEmail" :loading="reSendLoading"
                     :disabled="forgetNextLoading" v-if="forgetStep===1">
                重新发送
              </v-btn>
              <v-spacer/>
              <v-btn depressed color="primary" @click="forgetNext" :loading="forgetNextLoading"
      ...
      return new Promise((resolve, reject) => {
        ...
        (async () => {
          ...
          if (apiName === 'sendForgetEmailApi') {
            ...
            res = await getForgetPassTokenApi({
              username: this.formForget0.username,
              code: this.formForget1.code
          ...
          if (res.status === 200) {
            ...
            this.alertShow('server', {
              color: res['errorType'] ? res['errorType'] : 'info', msg: [
                ...this.alertObj.msg.filter(item => {
                  if (item.strong === "重置密码：") return item
                }), {strong: '提示：', text: res['msg']}
              ]
            })
            reject(res)
          } else {
            this.alertShow('server', {
              color: 'error', type: 'error', msg: [
                ...this.alertObj.msg.filter(item => {
                  if (item.strong === "重置密码：") return item
                }), {strong: '错误：', text: '发生未知错误'}
              ]
            })
            reject(res)
          }
