  ...
  <div id="Menu">
    <v-sheet elevation="0" color="rgba(255,255,255,0)">
      ...
      <v-data-table elevation="1" class="menuTable dataTable" fixed-header multi-sort show-select
        ...
        <template v-slot:top>
          <!--   编辑、新增对话框   -->
          <v-dialog persistent v-model="formDialog" :max-width="form.type===2?'600px':'800px'">
            <v-card>
              <v-card-title>
                          ...
                          <v-col v-if="!hideType[form.type].includes('pid')" cols="12">
                            <v-combobox :loading="loadingTree" label="上级菜单" hide-selected
                              ...
                              <template v-slot:no-data>
                                <v-treeview dense activatable hoverable :items="listTreeCur" return-object
        ...
        <template v-slot:[`item.allowAction`]="{ item }">
          <v-btn small icon class="mr-2" @click="openFormDialog(item,true)" title="新增子级" color="primary"
                 :loading="loadingTree">
  ...
  methods: {
    ...
    async fetchData(preEditObj) {
      ...
      if (res.status === 200) {
        ...
        //传来了上一次编辑的对象
        if (preEditObj) {
          let preEditPidObj = list.find(item => item.id === preEditObj.pid)
          let curObj = list.find(item => item.id === parseInt(preEditObj.id))
          //找到上次点击添加的菜单
          if (curObj && preEditPidObj && list[preEditPidObj.index + 1].level > preEditPidObj.level) {
            if (list[preEditPidObj.index + 2].level > preEditPidObj.level) {
              //假设上一子级是展开的，那么这次父级必定含有上次子级+现在的(>2)，如果任意一个子级展开，说明上次是展开的
              curObj.allowAction = list[preEditPidObj.index + 1].allowAction || list[preEditPidObj.index + 2].allowAction
            }
          }
        }
    ...
    openFormDialog(item, isAddChildren = false) {
      ...
      //设置自身ID禁止选
      let listTreeCur = this.$deepClone(this.listTree)

      function dfs(nodes, targetId) {
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (node.id === targetId) {
            ...
            let isFound = dfs(node.children, targetId)
            if (isFound) return isFound
          }
        }
      }

      item && dfs(listTreeCur, item.id)
      this.listTreeCur = listTreeCur
    ...
    async submitForm() {
      ...
      let res = await submitFun(data).then(res => res).catch(err => err)
      this.loadingSubmit = false;
      this.fetchData(res.result);
