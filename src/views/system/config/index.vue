  ...
  <div id="configs">
    <v-sheet elevation="0" color="rgba(255,255,255,0)">
      ...
      <div class="filterToolbar" v-show="filterToolbar">
        <v-text-field label="键" hint="键" style="flex: 0 1 200px"  dense
                      v-model="searchOption.key" clearable outlined hide-details
        ></v-text-field>

        <v-text-field label="描述" hint="描述" style="flex: 0 1 200px"  dense
                      v-model="searchOption.description" clearable outlined hide-details
        ></v-text-field>
        ...
        <v-menu v-model="FilterMenuCreateTime" :close-on-content-click="false" transition="scale-transition" offset-y
          ...
          <v-date-picker first-day-of-week="1" range v-model="searchOption.createTime" clearable></v-date-picker>
        </v-menu>

        <v-menu v-model="FilterMenuUpdateTime" :close-on-content-click="false" transition="scale-transition" offset-y
                min-width="auto">
          <template v-slot:activator="{ on, attrs }">
            <v-text-field label="更新时间" style="flex: 0 1 290px" outlined dense readonly hide-details
                          v-model="searchOption.updateTime" v-bind="attrs" v-on="on" clearable
            />
          </template>
          <v-date-picker first-day-of-week="1" range v-model="searchOption.updateTime" clearable></v-date-picker>
        </v-menu>
      ...
      <v-row>
        <v-col cols="auto">
          <div class="batchOperationBtn">
            ...
            <v-btn depressed color="warning" small :href="$addBaseURL(`/config/export`)" target="_blank">
      ...
      <!--   数据列表   -->
      <v-data-table elevation="1" class="configTable dataTable" fixed-header multi-sort show-select
        ...
        <template v-slot:top>
          <!--   编辑、新增对话框   -->
          <v-dialog persistent v-model="formDialog" max-width="800px">
            <v-card>
              <v-card-title>
                ...
                <v-container>
                  <v-form :disabled="loadingSubmit">
                    <v-row no-gutters>
                      <!--键-->
                      <v-col cols="12">
                        <v-text-field v-model="form.key" label="键" ></v-text-field>
                      </v-col><v-spacer/>
                      <!--描述-->
                      <v-col cols="12" >
                      ...
                      <!--值-->
                      <v-col cols="12">
<!--                        <v-textarea name="value" v-model="form.value" label="值" filled :rows="3"></v-textarea>-->
                        <v-data-table :headers="[{text: '键', value: 'k'},{text: '值', value: 'v'}]" :items="form.valueMap" v-if="form.valueMap">
                          <template v-slot:[`item.v`]="props">
                            <v-edit-dialog large :return-value.sync="props.item.v" save-text="保存" cancel-text="取消">
                              <div>{{ props.item.v }}</div>
                              <template v-slot:input>
                                <div class="mt-4 text-h6">值</div>
                                <v-text-field v-model="props.item.v" single-line autofocus/>
                              </template>
                            </v-edit-dialog>
                          </template>
                        </v-data-table>
...
<script>
import qs from "qs";
import {addConfigApi, delConfigApi, editConfigApi, readConfigApi, configListApi} from "@/api/modules";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: '键', value: 'key',initHide:false},
        {text: '值', value: 'value',initHide:true},
      ...
      searchSqlType: {
        equal: ['status',],/* 精确查询 */
        strLike: ['key','value', 'description',  'createUser.username'],/* 模糊查询 */
  ...
  methods: {
    ...
    async fetchData() {
      ...
      this.loading = true
      let res = await configListApi(data).catch(err => err)
      this.loading = false
      if (res.status === 200) {
        let list = [...res.list]
    ...
    async readData(item) {
      ...
      let res = await readConfigApi(data).catch(err => err)
    ...
    async deleteItemConfirm() {
      ...
      let res = await delConfigApi(data).then(res => res).catch(err => err).finally(() => {
        this.fetchData();
      })
    ...
    async openFormDialog(item) {
      if (this.$notEmpty(item) && this.$notEmpty(item['id'])) {
        ...
        this.form.valueMap = res.info.value&&Object.entries(res.info.value).map(item=>({k:item[0],v:item[1]}))
      }
    ...
    async submitForm() {
      ...
      data.value = {}
      data.valueMap && data.valueMap.forEach(item=>{
        data.value[item['k']]=item['v']
      })
      delete data.valueMap
      this.loadingSubmit = true;
      let submitFun = this.$notEmpty(data.id) ? editConfigApi : addConfigApi;
